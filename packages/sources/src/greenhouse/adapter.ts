...
export class GreenhouseAdapter implements SourceAdapter {
  ...
  public async *discover(boardId: string): AsyncIterableIterator<SourceOpportunity> {
    ...
    if (!response.ok) {
      throw new Error(`Greenhouse API returned ${response.status} ${response.statusText} for ${url}`);
    }

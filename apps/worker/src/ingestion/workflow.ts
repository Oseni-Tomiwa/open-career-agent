    ...
    'source.greenhouse.discover': async (task: BackgroundTask) => {
      ...
      for (const board of boards) {
        for await (const record of adapter.discover(board)) {
          ...
          sourceRepo.persistListing(listingId, {
            sourceSystem: record.sourceSystem,
            sourceExternalId: record.sourceExternalId,
            ...(record.sourceUrl ? { sourceUrl: record.sourceUrl } : {})
          }, existingListing?.opportunityId ?? undefined, record.observedAt.getTime());
          ...
          if (!oppId) {
            ...
            oppRepo.createOpportunity(opportunityId(oppId as string));
            sourceRepo.associateListingWithOpportunity(listingId, opportunityId(oppId as string));
          }
          ...
          const rawHash = createHash('sha256').update(record.rawPayload).digest('hex');
          
          const existingObs = sourceRepo.findObservationByFingerprint(listingId, rawHash);
          const obsId = existingObs?.id ?? `so_${randomUUID()}`;

          if (!existingObs) {
            sourceRepo.persistObservation(obsId, listingId, {
          ...
          const latestSnapshot = oppRepo.getLatestSnapshot(opportunityId(oppId as string));

          if (!latestSnapshot || latestSnapshot.fingerprint !== snapshotFingerprint) {
            const snapId = snapshotId(`snap_${randomUUID()}`);
            oppRepo.appendSnapshot({
              id: snapId,
              opportunityId: opportunityId(oppId as string),
              title: normalized.title,
              organization: normalized.organization,
              content: normalized.content,
              fingerprint: snapshotFingerprint,
              ...(normalized.location ? { location: normalized.location } : {}),
              ...(normalized.workModel ? { workModel: normalized.workModel } : {}),
              ...(normalized.employmentType ? { employmentType: normalized.employmentType } : {}),
              ...(normalized.compensation ? { compensation: normalized.compensation } : {}),
